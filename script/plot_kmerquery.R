...
for (cdataset in levels(d$dataset)) {
    # Data subsetting
    ds <- d[d$dataset == cdataset, c('seedlen', 'pathno', 'chunksize', 'patched', 'stepsize', 'seedfindingtime')]
    ds <- na.omit(ds)
    # Removing duplicates
    ...
    t <- tibble(chunksize=factor(uds$chunksize),
                ...
                kmerquery=(uds$seedfindingtime / ((col_readslen - uds$seedlen + 1) * col_nofreads)))
    g <- ggplot(t, aes(x=chunksize, y=kmerquery, fill=npath)) +
         ...
         theme_bw()
    ggsave(paste('kmerquery_', cdataset, '.pdf', sep=''))
}
