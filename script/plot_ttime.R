...
for (cdataset in levels(d$dataset)) {
    # Data subsetting
    ds <- d[d$dataset == cdataset, c('pathno', 'chunksize', 'patched', 'stepsize', 'traversetimeperchunk')]
    ds <- na.omit(ds)
    # Removing duplicates
    ...
    t <- tibble(chunksize=factor(uds$chunksize),
                ...
                ttime=(uds$traversetimeperchunk * (opt$nofreads / uds$chunksize) / (3600 * 10^6)))
    g <- ggplot(t, aes(x=chunksize, y=ttime, fill=npath)) +
         ...
         labs(title=opt$title,
              ...
              y='Traverse time (h)',
              fill='Paths') +
         scale_y_continuous(limits=c(0,max(t$ttime)*1.1), expand=c(0,0)) +
         theme_bw()
    ggsave(paste('ttime_', cdataset, '.pdf', sep=''))
}
