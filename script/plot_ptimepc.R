...
p_title <- 'Path query time per chunk'
...
for (cdataset in levels(d$dataset)) {
    # Data subsetting
    ds <- d[d$dataset == cdataset & d$chunksize < 10000, c('pathno', 'chunksize', 'stepsize', 'patched', 'pathquerytimeperchunk')]
    # Removing duplicates
    ...
    t <- tibble(npath=uds$pathno,
                ...
                ptime=(uds$pathquerytimeperchunk/10^6))
    g <- ggplot(t, aes(x=npath, y=ptime, colour=chunk, group=chunk)) +
         geom_point() +
         geom_line() +
         ...
         labs(title=opt$title,
              ...
              x='Paths',
              y='Path query time per chunk (s)',
              colour='Chunk size') +
         scale_x_continuous(breaks=unique(t$npath)) +
         theme_bw()
    ggsave(paste('ptimepc_', cdataset, '.pdf', sep=''))
}
